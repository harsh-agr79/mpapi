...
<head>
    ...
    <style>
        ...
        .info-table td, .items-table th, .items-table td, .amounts-table td {
...
<div class="section">
    ...
    <table class="amounts-table">
        <tr><td class="label">Sub-total</td><td class="value">NPR {{ number_format($order->total_amount, 2) }}</td></tr>
        <tr><td class="label">Discount</td><td class="value">NPR {{ number_format($order->discount, 2) }}</td></tr>
        <tr><td class="label">Total</td><td class="value">NPR {{ number_format($order->discounted_total, 2) }}</td></tr>
        @if ($order->coupon_code)
        <tr><td class="label">Coupon Code</td><td class="value">{{ $order->coupon_code }}</td></tr>
        <tr><td class="label">Coupon Discount</td><td class="value">NPR {{ number_format($order->coupon_discount, 2) }}</td></tr>
        @endif
        <tr><td class="label">Delivery Fee</td><td class="value">NPR {{ number_format($order->delivery_charge, 2) }}</td></tr>
        <tr><td class="label">Grand Total</td><td class="value warning">NPR {{ number_format($order->net_total, 2) }}</td></tr>
    </table>
</div>
