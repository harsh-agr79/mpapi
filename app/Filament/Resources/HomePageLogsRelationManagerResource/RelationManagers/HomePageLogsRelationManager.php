...
class HomePageLogsRelationManager extends RelationManager
{
        ...
        return ActivityLog::query()
            ->where(function ($query) use ($purchase) {
                $query->where(function ($q) use ($purchase) {
                    $q->where('table_name', 'home_pages')
                        ->where('primary_key_value', $purchase->id);
                })->orWhere(function ($q) use ($purchase) {
                    $q->where('table_name', 'home_page_covers');
                })->orWhere(function ($q) use ($purchase) {
                    $q->where('table_name', 'home_page_image_blocks');
                })->orWhere(function ($q) use ($purchase) {
                    $q->where('table_name', 'home_page_sliders');
                })->orWhere(function ($q) use ($purchase) {
                    $q->where('table_name', 'home_page_supports');
                });
            });
    }
