...
class AboutUsLogsRelationManager extends RelationManager
{
        ...
        return ActivityLog::query()
            ->where(function ($query) use ($purchase) {
                $query->where(function ($q) use ($purchase) {
                    $q->where('table_name', 'about_us')
                        ->where('primary_key_value', $purchase->id);
                })->orWhere(function ($q) use ($purchase) {
                    $q->where('table_name', 'about_us_cards');
                })->orWhere(function ($q) use ($purchase) {
                    $q->where('table_name', 'about_us_image_blocks');
                })->orWhere(function ($q) use ($purchase) {
                    $q->where('table_name', 'team_members');
                });
            });
    }
